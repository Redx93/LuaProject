...
bool Shader::InitializePixelShader(Microsoft::WRL::ComPtr<ID3D11Device>& device, std::wstring shaderpath)
{
	if (!PS.Initialize(device, shaderpath))
		return false;
	return true;
}

bool Shader::InitializeVertexShader(Microsoft::WRL::ComPtr<ID3D11Device>& device, std::wstring shaderpath, D3D11_INPUT_ELEMENT_DESC* layoutDesc, UINT numElements)
{
	if (!VS.Initialize(device, shaderpath, layoutDesc, numElements))
		return false;
	return true;
}
